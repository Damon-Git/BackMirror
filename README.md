...
线上隐私说明页：https://backmirror-production.up.railway.app/privacy.html

- BackMirror 不要求注册账号，不提供云相册，也不会把照片或视频保存到服务端。
- 媒体流通过 WebRTC 在手机浏览器和电脑浏览器之间传输。
- Node 服务只处理页面文件、随机房间 ID、二维码和 WebRTC 连接信令。
- 拍摄的图片在手机浏览器中生成，再通过 WebRTC DataChannel 发送到电脑浏览器。
- 部署平台可能记录 IP 地址、访问时间、URL、User-Agent、错误日志等基础运行信息。
- 后背、皮肤、身体局部、健康相关照片可能非常敏感；请只拍摄你自己有权处理的内容，不要拍摄他人、未成年人或任何未经同意的内容。
- BackMirror 不是医疗器械，也不提供诊断、治疗、筛查或健康评估。
- 如果以后启用 TURN，中继服务可能会转发媒体流，隐私模型会发生变化，应在启用前单独说明。

## 当前限制

- 两台设备应处于同一局域网内。
- WebRTC 当前默认配置 STUN 服务；某些特殊网络环境仍可能阻止点对点连接。
